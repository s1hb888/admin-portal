...
export default function FruitsCRUD() {
  ...
  const fetchFruits = async () => {
    try {
      const res = await axios.get(`${API_BASE_URL}/api/fruits`);
      setFruits(res.data);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        const confirmUpdate = window.confirm("Are you sure you want to update this fruit?");
        if (!confirmUpdate) return;

        await axios.put(`${API_BASE_URL}/api/fruits/${editingId}`, formData);
        setEditingId(null);
      } else {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`${API_BASE_URL}/api/fruits/${id}`);
      fetchFruits();
    } catch (err) {
